...
class DownloadDb {
  ...
  /// Check if a track is actually downloaded (has a real local file, not just
  /// synced metadata from a cloud playlist backup with no file yet).
  Future<bool> isDownloaded(String videoId) async {
    final db = await database;
    final result = await db.query(
      'downloaded_tracks',
      where: 'videoId = ? AND filePath IS NOT NULL',
      whereArgs: [videoId],
      limit: 1,
    );
