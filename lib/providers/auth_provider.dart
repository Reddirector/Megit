...
class AuthNotifier extends Notifier<AuthState> {
  ...
  Future<void> _onAuthChanged(User? firebaseUser) async {
    ...
    try {
      ...
      if (!snap.exists) {
        await ref.set({
          ...
          'displayName': firebaseUser.displayName ?? 'Megit User',
    ...
    state = AuthState(
      user: firebaseUser,
      displayName: displayName ?? 'Megit User',
      photoURL: photoURL,
      loading: false,
    );
  }
  ...
  Future<void> loginWithGoogle() async {
    ...
    // Store accessToken for authenticated InnerTube/YTM calls
    state = state.copyWith(accessToken: googleAuth.accessToken);
  }

  /// Ensures we have a YouTube access token. 
  /// If logged in with Email, it links/signs-in to Google for the token.
  Future<String?> ensureYoutubeAccess() async {
    ...
    try {
      ...
      debugPrint('[Auth] YT Access Error: $e');
    }
    return null;
  }
  ...
  Future<void> signupWithEmail(
    ...
    // where _onAuthChanged writes "Megit User" before the name is set.
