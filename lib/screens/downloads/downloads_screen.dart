...
class _DownloadsScreenState extends ConsumerState<DownloadsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final layoutMode = settings.downloadsLayoutMode;
    final sortKey = settings.downloadsSortKey;
    final sortOrder = settings.downloadsSortOrder;
                          ...
                          Consumer(
                            builder: (context, ref, _) {
                              ...
                              return LayoutSelector(
                                currentMode: mode,
                                onModeChanged: (m) => ref.read(settingsProvider.notifier).setDownloadsLayoutMode(m),
                              );
                            },
                          ),
                        ],
              ...
              Material(
                ...
                child: InkWell(
                  ...
                  child: const Padding(
                    padding: EdgeInsets.symmetric(horizontal: 20, vertical: 14),
                    child: Row(children: [
                      Icon(LucideIcons.trash_2, size: 17, color: AppColors.danger), SizedBox(width: 14),
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Padding(
              ...
              child: Icon(sortOrder == 'asc' ? LucideIcons.arrow_up : LucideIcons.arrow_down,
                  size: 14, color: AppColors.textSecondary),
            ),
          ),
        ],
...
class _MasonryDownloadCard extends StatelessWidget {
  ...
  const _MasonryDownloadCard({required this.playlist, required this.height, required this.audio, required this.onTap, required this.onLongPress});

  @override
  Widget build(BuildContext context) {
    final isPlayingThis = audio.contextPlaylistId == playlist.id;
    final songsList = (playlist.songs as List<dynamic>).map((s) => s is Song ? s : Song.fromJson(s as Map<String, dynamic>)).toList();
                ...
                Positioned(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(playlist.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: Colors.white, height: 1.2)),
                        const SizedBox(height: 2),
                        Text('${playlist.songs.length} tracks', style: TextStyle(fontSize: 10, color: Colors.white.withValues(alpha: 0.6), fontWeight: FontWeight.w600)),
                      ],
                    ),
                  ),
                ),
