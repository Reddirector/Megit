...
class _PlaylistScreenState extends ConsumerState<PlaylistScreen> {
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    // Removed LayoutSelector as requested
                  ],
                ),
              ),
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    // Cover
                    Hero(
                      tag: 'pl-thumb-${widget.playlistId}',
                      child: playlist != null
                          ? PlaylistThumbnail(
                              playlist: playlist,
                              width: 130,
                    ...
                    if (!isOffline) ...[
                      ...
                      if (!isYtm && !isOffline) ...[
                        ...
                        GestureDetector(
                          ...
                          child: const Icon(LucideIcons.share_2, size: 20, color: AppColors.textSecondary),
                        ),
                      ],
                    ],
                    const Spacer(),
                ...
                child: Stack(
                  children: [
                    songsToRender.isEmpty
                        ...
                        : _buildPlaylistContent(songsToRender, audio, accent, coverThumb),
  ...
  Widget _buildPlaylistContent(List<Song> songs, AudioState audio, Color accent, String coverThumb) {
    // Reverting playlist to simple ListView for a clean, non-masonry look
    return ListView.builder(
      padding: const EdgeInsets.only(bottom: 120),
      itemCount: songs.length,
      itemBuilder: (context, i) {
        final song = songs[i];
        final isActive = audio.currentSong?.videoId == song.videoId;
        final songWithThumb = song.thumbnail.isEmpty && coverThumb.isNotEmpty ? song.copyWith(thumbnail: coverThumb) : song;
        return SongTile(
          song: songWithThumb,
          isPlaying: isActive,
          index: i,
          onTap: () {
            ...
            notifier.replaceQueue(songs.sublist(i + 1));
          },
          onLongPress: () => _showMenu(song, i, false),
          trailing: GestureDetector(
            onTap: () => _showMenu(song, i, false),
            child: const Padding(padding: EdgeInsets.all(8), child: Icon(LucideIcons.ellipsis_vertical, size: 16, color: AppColors.textSecondary)),
          ),
        );
      },
    );
...
// [DELETE] _buildQuadCover helper here
  ...
  void _sharePlaylist(String name, bool isYtm) {
    final id = widget.playlistId;
    final url = 'https://megit.app/playlist/$id';
    Share.share('Check out "$name" on Megit!\n$url');
  }
      ...
      builder: (_) => SongActionSheet(
        ...
        onRemove: () {
          ref.read(playlistProvider.notifier)
              .removeSongFromPlaylist(widget.playlistId, song.videoId);
        },
