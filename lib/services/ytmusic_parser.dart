...
/// Minimal YouTube Music InnerTube JSON parser for Megit.
class YtMusicParser {
  ...
  static Song? _parseResponsiveListItem(dynamic item) {
    try {
      ...
      // Secondary fallback for videoId if still null
      if (videoId == null) {
         final menu = renderer['menu']?['musicMenuRenderer']?['items'] as List?;
         if (menu != null) {
           for (final m in menu) {
              final vid = m['menuNavigationItemRenderer']?['navigationEndpoint']?['watchEndpoint']?['videoId'] ??
                          m['menuServiceItemRenderer']?['serviceEndpoint']?['playlistEditEndpoint']?['actions']?[0]?['setVideoId'] ??
                          m['menuServiceItemRenderer']?['serviceEndpoint']?['queueAddEndpoint']?['queueTarget']?['videoId'];
              if (vid != null) {
          ...
          for (final run in subRuns) {
            ...
            if (effectivePart == 0) {
               ...
               if (RegExp(r'^\d+:\d+$').hasMatch(text)) {
                 ...
                 if (!RegExp(r'^\d{4}$').hasMatch(text) && !text.toLowerCase().contains('views')) {
                    parsedAlbum += text;
    ...
    try {
      ...
      for (var section in contents) {
        final items = (section['musicPlaylistShelfRenderer']?['contents'] ??
                       ...
                       (section.containsKey('musicResponsiveListItemRenderer') ? [section] : [])
                       ) as List;
        for (var item in items) {
          final song = _parseResponsiveListItem(item);
          if (song != null && song.videoId.isNotEmpty) tracks.add(song);
      ...
      // Path 4: Direct shelf at top of contents
      if (tracks.isEmpty) {
         final shelf = data['contents']?['singleColumnBrowseResultsRenderer']
             ?['tabs']?[0]?['tabRenderer']?['content']?['musicPlaylistShelfRenderer'];
         if (shelf != null) {
           final items = shelf['contents'] as List? ?? [];
           for (var item in items) {
             final song = _parseResponsiveListItem(item);
  ...
  static List<Playlist> parseLibraryPlaylists(Map<String, dynamic> data) {
    ...
    try {
      final grid = data['contents']?['singleColumnBrowseResultsRenderer']
      ...
      for (final item in items) {
        ...
        results.add(Playlist(
          ...
          description: _getText(renderer['subtitle']) ?? '',
          thumbnail: _extractThumb(renderer),
          type: 'YTM_PLAYLIST',
          songs: [],
          totalTracks: 0,
      ...
      debugPrint('[YtMusicParser] parseLibraryPlaylists error: $e');
    }
    return results;
  }
  ...
  static Artist parseArtist(Map<String, dynamic> data, String browseId) {
    ...
    try {
      ...
      description = _getText(header?['description']) ?? '';
      thumbnail = _extractThumb(header);
      subscribers = _getText(header?['subscriptionButton']?['subscribeButtonRenderer']?['subscriberCountText']) ?? '';
      ...
      for (var section in contents) {
        ...
        if (carousel != null) {
          ...
          final targetList = headerText.contains('single') ? singles : albums;
          ...
          for (var i in carousel['contents'] ?? []) {
            ...
            if (bId != null) {
              targetList.add(ArtistAlbum(
                browseId: bId,
                title: _getText(renderer['title']) ?? '',
                year: _getText(renderer['subtitle']) ?? '',
                thumbnail: _extractThumb(renderer),
                type: headerText.contains('single') ? 'SINGLE' : 'ALBUM',
              ));
            }
          }
        }
